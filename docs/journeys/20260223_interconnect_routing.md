...
- **`mod.rs`**: `NMPGC` owns the `Network` and `LineTopology`. Each tick: (1) processors execute, yielding outgoing messages; (2) messages are injected into the network with routes computed from the topology; (3) the network ticks, advancing all in-flight messages; (4) arrived messages are delivered to recipient inboxes. Termination requires all processors idle **and** no in-flight messages. Stats include per-link throughput.
...
The `Topology` trait is topology-agnostic. `LineTopology` is the only implementation, but ring and fully-connected topologies can be added by implementing `get_route()`, `get_links()`, and `get_per_hop_latency()`. The `Network` struct requires no modification—it operates on arbitrary routes.
...
Simulation on `fop/heapdump.2.binpb.zst`, 8 processors, Naive memory model:

| Metric | Master | Current | Delta |
| :--- | ---: | ---: | ---: |
| **Total Cycles** | 1,802,370 | 1,687,031 | −6.4% |
| **Utilization** | 0.818 | 0.812 | −0.006 |
| **Busy Ticks (sum)** | 11,789,730 | 10,957,484 | −7.1% |
| **Read Hit Rate** | 0.714 | 0.717 | +0.003 |
| **Marked Objects** | 93,180 | 93,180 | — |

The 6.4% reduction in total cycles is expected: the sender now stalls for only 2 cycles (DIMM-to-rank handoff) instead of the full end-to-end latency (4–12 cycles). This frees processors to do productive work sooner while messages transit the network asynchronously. Utilization drops slightly because the faster completion means proportionally less of the total time is spent busy. Marked objects remain identical, confirming correctness.
